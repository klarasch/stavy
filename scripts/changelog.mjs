#!/usr/bin/env node
// Human-readable changelog between two versions of protoscope.json.
//
...
function gitShow(ref) {
  try {
    return JSON.parse(execSync(`git show ${ref}:protoscope.json`, { encoding: "utf8", stdio: ["ignore", "pipe", "ignore"] }))
  } catch {
    return null
  }
}
...
const head = headArg && headArg !== "--working" ? gitShow(headArg) : JSON.parse(readFileSync("protoscope.json", "utf8"))
