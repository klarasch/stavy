...
test.describe("Manager reviews and approves", () => {
  ...
  test("2. Open the submitted expense", async ({ page }) => {
    ...
    await expect.soft(page).toHaveURL(new RegExp("/expenses/exp-2101\\/?(?:\\?(?=(?:(?:[^#]*&)?role=manager(?:&|#|$)))(?=(?:(?:[^#]*&)?lifecycle=submitted(?:&|#|$))|(?![^#]*(?:^|&)lifecycle=))(?=(?:(?:[^#]*&)?locale=en-US(?:&|#|$))|(?![^#]*(?:^|&)locale=))(?=(?:(?:[^#]*&)?overlay=none(?:&|#|$))|(?![^#]*(?:^|&)overlay=))[^#]*)(?:#.*)?$"))
  })

  test("3. Approve it", async ({ page }) => {
    await page.goto("/expenses/exp-2101?role=manager&lifecycle=submitted&locale=en-US&overlay=none")
    ...
    await expect(page).toHaveURL(new RegExp("/expenses/exp-2101\\/?(?:\\?(?=(?:(?:[^#]*&)?role=manager(?:&|#|$)))(?=(?:(?:[^#]*&)?lifecycle=approved(?:&|#|$)))(?=(?:(?:[^#]*&)?locale=en-US(?:&|#|$))|(?![^#]*(?:^|&)locale=))(?=(?:(?:[^#]*&)?overlay=none(?:&|#|$))|(?![^#]*(?:^|&)overlay=))[^#]*)(?:#.*)?$"))
  })

  test("4. Approved", async ({ page }) => {
    await page.goto("/expenses/exp-2101?role=manager&lifecycle=approved&locale=en-US&overlay=none")
