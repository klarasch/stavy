# Stavy — an open standard for AI-assisted prototyping (v0.1, draft)

Stavy is a convention for organizing vibe-coded prototypes so that they stay
...
`src/stavy/` unchanged and implementing only §2.1 and §2.2. Re-theming the
...
- **Authoring in dev** (optional): a dev server MAY expose an endpoint that
  ...
  (reference: `POST /__stavy/annotation`), so designers annotate without
...
- **Level 0 — manifest only.** The workspace has an accurate `stavy.json`.
